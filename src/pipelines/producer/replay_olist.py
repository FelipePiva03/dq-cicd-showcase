...
def attach_effective_ts(table: str, df: DataFrame, orders_df: DataFrame) -> DataFrame:
    ...
    """
    if table == "orders":
        return df.withColumn("_effective_ts", F.to_date("order_purchase_timestamp"))
    ...
    if table == "order_reviews":
        return df.withColumn("_effective_ts", F.to_date("review_creation_date"))
...
def replay(
    ...
    keyed = {t: attach_effective_ts(t, load_raw(spark, source_root, t), orders_df) for t in tables}

    per_table_bounds = [
        k.agg(F.min("_effective_ts"), F.max("_effective_ts")).first() for k in keyed.values()
    ]
    ...
    while cursor <= global_max:
        ...
        for table, df in keyed.items():
            batch_df = df.filter(
                (F.col("_effective_ts") >= F.lit(cursor)) & (F.col("_effective_ts") < F.lit(end))
            )
